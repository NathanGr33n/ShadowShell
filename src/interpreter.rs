...
fn interpret_and_or(
    list: &crate::parser::AndOrList,
    shell: &mut Shell,
) -> InterpretOutcome {
    let mut outcome = interpret_pipeline(&list.first, shell);
    if matches!(outcome, InterpretOutcome::Exit(_) | InterpretOutcome::Return(_)) {
        return outcome;
    }
    ...
    for cmd in &pipeline.commands {
        match cmd {
            Command::Simple(simple) => {
                ...
                } else if is_builtin(&expanded.program) || shell.functions.contains_key(&expanded.program)
                {
                    eprintln!(
...
fn call_function(name: &str, args: &[String], shell: &mut Shell) -> InterpretOutcome {
    ...
    shell
        .env
        .set_positionals(args.iter().cloned().collect::<Vec<_>>());
...
fn run_compound(compound: &CompoundCommand, shell: &mut Shell) -> InterpretOutcome {
    match compound {
        ...
        CompoundCommand::If {
            ...
            if matches!(cond, InterpretOutcome::Exit(_) | InterpretOutcome::Return(_)) {
                return cond;
            }
            if cond.code() == 0 {
        ...
        CompoundCommand::Case { word, arms } => {
            let env_snap = shell.env.clone();
            let subject = expand_word_unsplit(word, &env_snap, |b| run_command_substitution(b, shell));
            for arm in arms {
                for pat in &arm.patterns {
                    let env_snap = shell.env.clone();
                    let pat_s = expand_word_no_glob(pat, &env_snap, |b| run_command_substitution(b, shell));
...
mod tests {
    ...
    fn export_reaches_child() {
        ...
        let path = dir.join(format!("shadowshell-interp-export-{}.txt", std::process::id()));
        let src = format!(
